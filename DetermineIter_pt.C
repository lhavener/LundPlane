void DetermineIter(std::string file, std::string file_rw, std::string var, int pt1, int pt2)
{
  ...
  for (int i = 1; i <= 9; i++)
    {
      ...
      ss << "_proj";
      TH1D* h1_nom = (TH1D*)h2_nom->ProjectionZ(ss.str().c_str(), 1, 7, 2, 12);
      ...
      ss << "_proj";
      TH1D* h1_rw = (TH1D*)h2_rw->ProjectionZ(ss.str().c_str(), 1, 7, 2, 12);
	...
	ss << "_proj";
	h1_iterdown = (TH1D*)h2_iterdown->ProjectionZ(ss.str().c_str(), 1, 7, 2, 12);
	...
	ss << "_proj";
	h1_iterup = (TH1D*)h2_iterup->ProjectionZ(ss.str().c_str(), 1, 7, 2, 12);
  ...
  ss.str("");
  ss << "figures/IterationsDet_pt_11_22_20.pdf";
