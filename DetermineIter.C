void DetermineIter(std::string file, std::string file_rw, std::string var, int pt1, int pt2)
{
  ...
  for (int i = 1; i <= 9; i++)
    {
      ...
      ss << "_proj";
      TH1D* h1_nom = (TH1D*)h2_nom->ProjectionX(ss.str().c_str(), 2, 12, pt1, pt2);
      ...
      ss << "_proj";
      TH1D* h1_rw = (TH1D*)h2_rw->ProjectionX(ss.str().c_str(), 2, 12, pt1, pt2);
	...
	ss << "_proj";
	h1_iterdown = (TH1D*)h2_iterdown->ProjectionX(ss.str().c_str(), 2, 12, pt1, pt2);
	...
	ss << "_proj";
	h1_iterup = (TH1D*)h2_iterup->ProjectionX(ss.str().c_str(), 2, 12, pt1, pt2);
	h1_iterup->Scale(1./h1_iterup->Integral(), "width");
      ...
      double iter = 0.;
      double tot = 0.;
      for (int j = 1; j <= h1_nom->GetNbinsX()-1; j++)
	{
  ...
  ss.str("");
  ss << "figures/IterationsDet_11_22_20.pdf";
