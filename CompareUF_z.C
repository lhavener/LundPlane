void CompareUF(std::string file1, std::string file2, std::string tag1, std::string tag2, std::string date, int x1, int x2, int y1, int y2)
{
  ...
  TH1D* h1_uf1 = (TH1D*)h2_uf1->ProjectionZ("h1_uf1", x1, x2, y1, y2);
  h1_uf1->Sumw2();
  ...
  TH1D* h1_uf2 = (TH1D*)h2_uf2->ProjectionZ("h1_uf2", x1, x2, y1, y2);
  h1_uf2->Sumw2();
  ...
  h1_uf1->GetXaxis()->SetTitle("#it{p}_{T}");
  h1_uf1->GetYaxis()->SetTitle("#frac{1}{N_{jet}}#frac{dN}{d#it{p}_{T}}");
  h1_uf1->SetTitle("");
  ...
  std::stringstream ss;
  ss << "figures/CompUF3Dz_" << tag1 << "_" << tag2 << "_"<< x1 << "to" << x2 << "_"<< y1 << "to" << y2 << "_" << date << ".pdf";
