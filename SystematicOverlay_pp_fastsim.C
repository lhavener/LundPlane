...
void SystematicPlots(std::string file, int pt1, int pt2, int y1, int y2, std::string var, bool fastsim)
{
  ...
  if(var == "KT") hnom_err->GetXaxis()->SetRangeUser(-1, 1.5);
       if(var == "dR") hnom_err->GetXaxis()->SetRangeUser(0, 1.4);
  ...
  for (int i = 0; i  < 6; i++)
      ...
      else 
	{
      ...
      //
      if(sys[i] == "herwig2fast"){
     double val=(hratio->GetBinContent(5)+hratio->GetBinContent(9))/2;
  hratio->SetBinContent(6,val);
  

  hratio->SetBinContent(7,val);
  

  hratio->SetBinContent(8,val);}
  
  //

      
      hratio->Divide(hnom);
      for(Int_t k=0;k<hratio->GetNbinsX();k++){
        cout<<"the content"<<k+1<<" "<<hratio->GetBinContent(k+1)<<endl;
      }
  ...
  if(!fastsim) ss << "figures/SysOverlay_" << var << "_" <<  pt1  <<  "to"<< pt2 << "_" << y1 << "to" << y2 << "_12_10_20.pdf";
  if(fastsim) ss << "figuresfastsim/SysOverlay_" << var << "_" <<  pt1  <<  "to"<< pt2 << "_" << y1 << "to" << y2 << "_12_10_20.pdf";
