...
void EvaluateSys(int pt1, int pt2, int y1, int y2, bool ifpp, bool fastsim, bool smooth)
{
 ...
 std::string file_matchup = "Unfold_nom_matchup_Dec5.root";
  std::string file_matchdown = "Unfold_nom_matchdown_Dec5.root";
  ...
  std::string file_fastsimpy="Unfold_FastSimPythia_March16.root";
  std::string file_fastsimhw="Unfold_FastSimHerwig_March16.root";
  
  std::string file_sherpa = "SherpaHistograms_Ahadic_March16.root";
  std::string file_sherpaL = "SherpaHistograms_lundstring_March16.root";
  ...
  h1_herwig2 = (TH1D*)Norm(h2_herwig, y1, y2, pt1, pt2, "h1_herwig2", h1_norm_herwig);
  ...
  cout<<"fastsimuncer"<<endl;

  //fastsim uncert////////
  if(fastsim){
  TFile* fherwigfast = new TFile(file_fastsimhw.c_str());
  TH3D* h2_herwigfast = (TH3D*)fherwigfast->Get(histname.c_str());
  std::cout << h2_herwigfast->Integral() << std::endl;
  h2_herwigfast->SetName("h2_herwigfast");
  TH1D* h1_norm_herwigfast = (TH1D*)fherwigfast->Get(hist1Dname.c_str());
  h1_norm_herwigfast->SetName("h1_norm_herwigfast");
  TH3D* heff_herwigfast = (TH3D*)fherwigfast->Get(effname.c_str());
  heff_herwigfast->SetName("heff_herwigfast");
  TH3D* heff_herwig_denfast = (TH3D*)fherwigfast->Get(effdenname.c_str());
  heff_herwig_denfast->SetName("heff_herwig_denfast");
  heff_herwigfast->Divide(heff_herwig_denfast);
  h2_herwigfast->Divide(heff_herwigfast);
  TH1D* heff_norm_herwigfast = (TH1D*)fherwigfast->Get(eff1Dname.c_str());
  heff_norm_herwigfast->SetName("heff_norm_herwigfast");
  TH1D* heff_norm_herwig_denfast = (TH1D*)fherwigfast->Get(eff1Ddenname.c_str());
  heff_norm_herwig_denfast->SetName("heff_norm_herwig_denfast");
  heff_norm_herwigfast->Divide(heff_norm_herwig_denfast);
  h1_norm_herwigfast->Divide(heff_norm_herwigfast);
  h1_herwig2fast = (TH1D*)Norm(h2_herwigfast, y1, y2, pt1, pt2, "h1_herwig2fast", h1_norm_herwigfast);
  histvec.push_back(h1_herwig2fast);


  TFile* fpythiafast = new TFile(file_fastsimpy.c_str());
  TH3D* h2_pythiafast = (TH3D*)fpythiafast->Get(histname.c_str());
  std::cout << h2_pythiafast->Integral() << std::endl;
  h2_pythiafast->SetName("h2_pythiafast");
  TH1D* h1_norm_pythiafast = (TH1D*)fpythiafast->Get(hist1Dname.c_str());
  h1_norm_pythiafast->SetName("h1_norm_pythiafast");
  TH3D* heff_pythiafast = (TH3D*)fpythiafast->Get(effname.c_str());
  heff_pythiafast->SetName("heff_pythiafast");
  TH3D* heff_pythia_denfast = (TH3D*)fpythiafast->Get(effdenname.c_str());
  heff_pythia_denfast->SetName("heff_pythia_denfast");
  heff_pythiafast->Divide(heff_pythia_denfast);
  h2_pythiafast->Divide(heff_pythiafast);
  TH1D* heff_norm_pythiafast = (TH1D*)fpythiafast->Get(eff1Dname.c_str());
  heff_norm_pythiafast->SetName("heff_norm_pythiafast");
  TH1D* heff_norm_pythia_denfast = (TH1D*)fpythiafast->Get(eff1Ddenname.c_str());
  heff_norm_pythia_denfast->SetName("heff_norm_pythia_denfast");
  heff_norm_pythiafast->Divide(heff_norm_pythia_denfast);
  h1_norm_pythiafast->Divide(heff_norm_pythiafast);
  h1_pythia2fast = (TH1D*)Norm(h2_pythiafast, y1, y2, pt1, pt2, "h1_pythia2fast", h1_norm_pythiafast);
 
   

  std::pair<TH1D*, TH1D*> herwigfast = CalculateDeltaCut(h1_pythia2fast, h1_herwig2fast, h1_herwig2fast, true);

  if(smooth){

  double val=(herwigfast.first->GetBinContent(5)+herwigfast.first->GetBinContent(9))/2;
  herwigfast.first->SetBinContent(6,val);
  herwigfast.second->SetBinContent(6,val);

  herwigfast.first->SetBinContent(7,val);
  herwigfast.second->SetBinContent(7,val);

   herwigfast.first->SetBinContent(8,val);
  herwigfast.second->SetBinContent(8,val);

  }


  histvec_up.push_back(herwigfast.first);
  histvec_down.push_back(herwigfast.second);
  ...
  name << "Resultslnkt_";
  name << y1 << "to" << y2 << "_" << pt1 <<"to" <<pt2 <<"fastsim"<<fastsim << "_Dec21.root";
  ...
  h1_sherpaL->Write();
  h1_herwig->Write();
  if(!fastsim) h1_herwig2->Write();
  
  if(fastsim){h1_herwig2fast->Write();
    h1_pythia2fast->Write();}
...
TGraphAsymmErrors* MakeAsymmError(std::vector<TH1D*> histvec_up, std::vector<TH1D*> histvec_down, TH1D* h)
{
  ...
  ss.str("");
  cout<<"size of the vector"<<histvec_up.size()<<endl;
...
std::pair<TH1D*, TH1D*> CalculateDeltaCut(TH1D* h, TH1D* h_up, TH1D* h_down, bool sym)
{
  ...
  for (int i = 1; i <= h->GetNbinsX(); i++)
      ...
      else 
	{
      ...
      cout<<delta_low<<" "<<delta_high<<endl;
    }
